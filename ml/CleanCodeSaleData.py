...
def clean_data(df):
    try:
        df = df.drop_duplicates(subset=["property_id","all_image_url"])
...
def run_pipeline():
    ...
    df = load_data("ml\SaleData.csv", encoding="utf-8-sig")
    df = clean_data(df)
    df = feature_engineering(df)
    #df.to_csv("saledataclened.csv", index=False, encoding="utf-8-sig")
